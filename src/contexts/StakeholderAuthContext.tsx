...
const SESSION_START_KEY = 'stakeholder_session_start';
...
const fetchWithRetry = async (
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      console.log(`[Stakeholder Auth] Fetch attempt ${attempt + 1} failed:`, lastError.message);
...
export const StakeholderAuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const isSessionExpired = (): boolean => {
    ...
    return Date.now() - parseInt(startTime) > SESSION_TIMEOUT_MS;
  };

  // Clear session with timeout logging
  const clearSessionWithReason = (reason: string) => {
    console.log(`[Stakeholder Auth] Clearing session: ${reason}`);
  ...
  useEffect(() => {
    ...
    if (stored) {
      // Check timeout FIRST before restoring session
      if (isSessionExpired()) {
        clearSessionWithReason('Session expired (30 min timeout)');
      } else {
        try {
          const currentSession = JSON.parse(stored);
          console.log('[Stakeholder Auth] Loaded session from localStorage immediately');
          setStakeholderSession(currentSession);
          
          // Validate in background (non-blocking)
          validateInBackground(currentSession);
        } catch {
    ...
    const timeoutInterval = setInterval(() => {
      if (isSessionExpired()) {
        clearSessionWithReason('Session timeout (30 min)');
      }
    }, 60000);

    // Tab visibility warm-up for Safari CORS mitigation
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        // Check timeout when tab becomes visible
        if (isSessionExpired()) {
          clearSessionWithReason('Session expired while tab was hidden');
          return;
        }
        
        // Re-read from localStorage in case session changed
        const storedSession = localStorage.getItem(STAKEHOLDER_SESSION_KEY);
        if (storedSession) {
          try {
            const session = JSON.parse(storedSession);
            console.log('[Stakeholder Auth] Tab visible, warming up connection');
  ...
  const validateAndSetSession = async (key: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (data.valid && data.stakeholder) {
        ...
        // Record session start time for 30-minute timeout
